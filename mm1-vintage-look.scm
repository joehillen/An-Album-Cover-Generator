...
; mm1-vintage-look with file handling wrapper
(define (once-vintage-look filename new-filename)
   (let* ((image (car (gimp-file-load RUN-NONINTERACTIVE filename filename)))
          (drawable (car (gimp-image-get-active-layer image)))
          (new-layer 0)
   )
     (mm1-vintage-look image drawable 17 20 59 TRUE)
     (set! new-layer (car (gimp-image-merge-visible-layers image EXPAND-AS-NECESSARY)))
     (gimp-file-save RUN-NONINTERACTIVE image new-layer new-filename new-filename)
